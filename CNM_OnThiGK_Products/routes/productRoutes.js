...
========================= */
router.post("/add", upload.single("url_image"), async(req, res) => {
    try {
...
/* =========================
   5. UPDATE SẢN PHẨM
========================= */
router.put("/edit/:id", upload.single("url_image"), async(req, res) => {
    try {
        const { id } = req.params;
        const { name, price, unit_in_stock } = req.body;

        let updateExpression =
            "set #name = :name, price = :price, unit_in_stock = :unit_in_stock";
...
/* =========================
   6. DELETE SẢN PHẨM
========================= */
// SỬA TỪ router.get THÀNH router.delete
router.delete("/delete/:id", async(req, res) => {
    try {
        const { id } = req.params;

        // 1. Lấy thông tin sản phẩm trước khi xóa để lấy URL ảnh
        const data = await docClient.send(
            new GetCommand({
                TableName: process.env.TABLE_NAME,
                Key: { id },
        ...
        // 2. Xóa ảnh trên S3 nếu có
        if (product && product.url_image) {
            // Tách lấy Key từ URL (Cẩn thận: Kiểm tra kỹ format URL của bạn)
